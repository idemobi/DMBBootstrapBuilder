...
namespace DMBBootstrapBuilder
{
    ...
    public sealed class TitleEffectsComposer : IIsCssClassComposer
    {
        ...
        public string BuildExtraAttributes(string encodedTitle)
        {
            ...
            if (_typewriter) sb.Append($" data-typewriter-chars=\"{encodedTitle.Length}\" data-typewriter-duration=\"{_typewriterDuration.ToString(ci)}\"");

            if (_scramble) sb.Append($" data-scramble-text=\"{encodedTitle}\" data-scramble-duration=\"{_scrambleDuration.ToString(ci)}\"");

            if (_glitchText) sb.Append($" data-text=\"{encodedTitle}\"");

            if (_splitColor && !_glitchText) sb.Append($" data-text=\"{encodedTitle}\"");
        ...
        public string BuildStyleValue()
        {
            ...
            if (_twisted) sb.Append($"transform:rotate({_twistAngle.ToString(ci)}deg);");
            ...
            if (_typewriter) sb.Append("overflow:hidden;white-space:nowrap;border-right:3px solid;width:0;");

            if (_letterCollapse) sb.Append($"animation:bb-letter-collapse {_letterCollapseDuration.ToString(ci)}s ease-out forwards;");
            ...
            if (_shake) sb.Append($"--bb-shake-speed:{_shakeSpeed.ToString(ci)}s;");
            ...
            if (_stamp) sb.Append($"--bb-stamp-duration:{_stampDuration.ToString(ci)}s;");
        ...
        public IReadOnlyList<string> BuildClasses()
        {
            ...
            if (_scramble) result.Add("bb-scramble");

            if (_neonGlow) result.Add("bb-title-neon");

            if (_glitchText) result.Add("bb-title-glitch");

            if (_blurReveal) result.Add("bb-title-blur-reveal");

            if (_colorCycle) result.Add("bb-title-color-cycle");

            if (_outline) result.Add("bb-title-outline");

            if (_shake) result.Add("bb-title-shake");

            if (_splitColor) result.Add("bb-title-split-color");

            if (_stamp) result.Add("bb-title-stamp");
